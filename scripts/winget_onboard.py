...
def generate_manifests(edition, version, url, desc):
    publisher = "ArchForm"
    app_name = f"ArchForm {edition.upper()}" if edition != 'standard' else "ArchForm"
    ...
    installer_manifest = {
        ...
        "Installers": [{
            ...
            "NestedInstallerFiles": [{
                "RelativeFilePath": f"{app_name}.exe" if edition != 'standard' else "ArchForm.exe",
                "PortableCommandAlias": edition if edition != 'standard' else "archform"
            }]
        }],
    ...
    locale_manifest = {
        ...
        "PrivacyUrl": "https://www.archform.com/terms-of-service",
        "Author": publisher,
        ...
        "Description": desc,
        "Tags": ["orthodontics", "dental", "design", edition],
...
def main():
    ...
    for _, row in df.iterrows():
        ...
        # FIXED: Correcting filename logic to match Winget specs
        for m_type, content in manifests.items():
            if m_type == "version":
                filename = f"{pkg_id}.yaml" # Should not be .version.yaml
            elif m_type == "locale":
                filename = f"{pkg_id}.locale.en-US.yaml" # Must include locale string
            else:
                filename = f"{pkg_id}.{m_type}.yaml" # .installer.yaml is correct
        ...
        pr_cmd = [
            ...
            "--body", f"Initial onboarding for {pkg_id} generated by community automation. Fixing manifest naming convention.",
            "--repo", WINGET_REPO,
